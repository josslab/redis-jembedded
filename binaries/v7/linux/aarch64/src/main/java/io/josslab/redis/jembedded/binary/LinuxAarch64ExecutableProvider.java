...
public class LinuxAarch64ExecutableProvider extends AbstractExecutableProvider {
  ...
  protected File doGetExecutable() {
    File tempDir = EnvironmentUtils.newTempDirForBinary("redis-jembedded-linux-aarch64" + EXECUTABLE_PROPERTY.jarVersion());
    try {
      return EnvironmentUtils.writeResourceToExecutableFile(LinuxAarch64ExecutableProvider.class, tempDir, "redis-server", "redis-server-" + EXECUTABLE_PROPERTY.binaryVersion());
    } catch (IOException e) {
