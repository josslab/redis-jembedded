...
public class MacOSAarch64ExecutableProvider extends AbstractExecutableProvider {
  ...
  protected File doGetExecutable() {
    File tempDir = EnvironmentUtils.newTempDirForBinary("redis-jembedded-macos-aarch64-");
    try {
      return EnvironmentUtils.writeResourceToExecutableFile(MacOSAarch64ExecutableProvider.class, tempDir, "redis-server");
    } catch (IOException e) {
