...
public abstract class RedisInstance implements Redis {
  ...
  protected RedisInstance(final String bindAddress, final int port, final List<String> args, final Pattern readyPattern,
                          final boolean forceStop, final Consumer<String> soutListener,
                          final Consumer<String> serrListener) {
    this.bindAddress = bindAddress;
    this.port = NetUtils.allocatePort(port);
    this.args = args;
  ...
  //
  //     /app/redis-server-7.2.5-linux-amd64-6.2.14-linux-amd64.tw.bak: error while loading shared libraries: libssl.so.3: cannot open
